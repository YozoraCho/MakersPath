...
SlashCmdList["MPIGN"] = function(msg)
  ...
  local f = _G.MakersPathFrame
  if f and f:IsShown() and RefreshList then RefreshList() end
end
...
local function ScoreItemWithBreakdown(iid, slotName, preKnownArmorTok, preKnownInvType)
  ...
  local total = (baseStatsScore or 0) + armorAdd + dpsAdd + statBonus + biasArmor + biasWeap + biasProf + biasGather
...
function ProfessionBias(entry)
  ...
  local profSpell = SKILLLINE_TO_SPELL[skillLine]
  if profSpell and pmap[profSpell] ~= nil then
    return 0.5
  end
  return 0
end
...
local function QueueGearFinderRescan(delay)
  if _rescanQueued then return end
  _rescanQueued = true
  C_Timer.After(delay or 0.25, function()
    ...
    if f and f:IsShown() and RefreshList then
      RefreshList()
    end
  end)
end
...
local function MakeSummaryRow(slotName, best, bestScore, eqScore, alts)
  local pct = 0
  if bestScore and bestScore > 0 then
    pct = math.max(0, math.min(1, (eqScore or 0) / bestScore))
  end
  if best then
    best.eqScore = eqScore
  end
...
function GearFinder:BuildSummaryAsync(onProgress, onDone)
  self._buildToken = (self._buildToken or 0) + 1
  local myToken = self._buildToken

  if self._summaryWorker then
    self._summaryWorker:SetScript("OnUpdate", nil)
  else
    self._summaryWorker = CreateFrame("Frame")
  end

  self:BeginSession()
  self._isBuildingSummary = true
  ...
  local function addOrReplaceRow(row)
    ...
    for _, slotName in ipairs(slots) do
      if rowBySlot[slotName] then
        rebuilt[#rebuilt+1] = rowBySlot[slotName]
      end
    end
    rows = rebuilt
  end

  self._summaryWorker:SetScript("OnUpdate", function(frame)
    if myToken ~= self._buildToken then
    ...
    while i <= #slots and (debugprofilestop() - start) < budgetMS do
      local slotName = slots[i]
...
function GearFinder:InvalidateSummary()
  self._summaryDirty = true
  self._lastSummary = nil
